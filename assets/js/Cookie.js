...
class Cookie {
    ...
    manage = {
        ...
        localCookies : function () {
            ...
            let cookies = JSON.parse(this.getCookie(name))
            return cookies
        },
        arrayCookies : function () {
            let name = Config.Cookies.preferences.name === undefined ? Default.name : Config.Cookies.preferences.name
            let cookies = this.getCookie(Config.Cookies.preferences.name) ? Object.entries(JSON.parse(this.getCookie(name)).cookies) : 'not consented yet'
            return cookies
        },
    ...
    consent = {
        ...
        clearCookies: () => {
            debugger
            this.manage.deleteCookie(this.settings.name)
        ...
        set: (key) => {
            ...
            if (key == false) {
                ...
                this.manage.setCookie(this.settings.name, key, this.settings.expire);
            }
        },
        //-------------------------------------------------------
        ...
        checkConfig: () => {
            ...
            if (!this.manage.getLocalStorage(this.defaultConsentName)){
                ...
                if (this.manage.getLocalStorage(this.defaultConsentName) === 'declined'){
                    ...
                    for ( i=0; i< this.configCookies.length; i++ ) {
        ...
        CookieSettingsElements: () => {
            ...
            document.body.after(div_cookie_wrapper)
                ...
                let floater = this.create.Element('div', { class: 'cookie_floater' })
                    ...
                    let cookie_img = this.create.Element('img', { alt: 'Cookie Privacy Settings', src: 'assets/img/cookie_1f36a.png' })
        ...
        bake: (preferences, action = 'setCookie', form = false) => {
            ...
            this.setConsentByForm = (preferences) => {
                ...
                for (let i=0; i < preferences.length; i++){
                    let n = preferences[i]
                    this.settings.cookies[n].wanted = true
                }
            }
            if (action === 'setCookie') {
                ...
                if(form === true) {
                    for (let i = 0; i < preferences.length; i++){
                        ...
                        manage.setLocalStorage(this.defaultConsentName, 1 ) // accepted
